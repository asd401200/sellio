...
let currentUser = null, products = [], orders = [];
let suppliers = [], mappings = [], supplierProducts = {};
let allUsers = [], allSuppliers = [], allMappings = [];
...
async function enterApp() {
  ...
  if (currentUser.role === 'admin') {
    ...
    await loadSuppliers(); await loadMappings();
  }
...
function renderProducts() {
  ...
  }).join('') : '<tr><td colspan="6" class="empty"><p>상품 가져오기를 눌러주세요</p></td></tr>';
...
async function loadAdminDash() {
  try {
    const [uR, sR, mR, srR] = await Promise.all([get('/admin/users'), get('/suppliers'), get('/admin/all-mappings'), get('/admin/supplier-requests')]);
    allUsers = (uR.users||[]).filter(u => u.role === 'seller'); allSuppliers = sR.suppliers||[]; allMappings = mR.mappings||[]; allSupReqs = srR.requests||[];
    $('ad-sellers').innerHTML = `${allUsers.length}<small>명</small>`;
    $('ad-mappings').innerHTML = `${allMappings.length}<small>건</small>`;
    renderAdminSellers(); renderAdminSupReqs();
...
function renderSellerOrders() {
  ...
  }).join('') : '<tr><td colspan="10" class="empty"><p>주문 불러오기를 눌러주세요</p></td></tr>';
